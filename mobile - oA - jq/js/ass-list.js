...
// 				删除一项任务
				$('#ass_list section.find-money-list article.fn-pt-10').find('aside:first a').on('click',function(){
					$(this).parent().parent().parent().remove();
				});
//				清空任务
				$('#del-all-ass').on('click',function(){
						$('#ass_list').empty();
				});
//				任务收藏
				$('#ass_list section.find-money-list h2.fn-mb-5').find('a i').on('click',function(){
					if($(this).hasClass('icon-shoucang')){
						$(this).removeClass('icon-shoucang');
						$(this).addClass('icon-shoucang1');

						$(this).parent().find('a>span').remove();
						var $al = $('<span style="color: red;position: absolute;left: 5px;top: 0px;">-1</span>');
						$(this).parent().append($al);
						$al.animate({top: '10px',opacity: 0},1000,function(){
							$al.remove();
						});
					}else if($(this).hasClass('icon-shoucang1')){
